"""
VisionGPT2 image captioning (ViT encoder + GPT-style decoder with cross-attention).

This project extends the earlier `MyLLM` work by reusing core Transformer building blocks:
- `MyLLM.ch3.MultiHeadAttention` (causal self-attention for text)
- `MyLLM.ch4.FeedForward` and `MyLLM.ch4.LayerNorm` (MLP + normalization)

High-level idea:
- A pretrained ViT encodes an image into a sequence of visual tokens (patch tokens + CLS token).
- A GPT-style decoder generates the caption autoregressively.
...
def collate_fn(batch):
    """Stack images and pad token sequences for a batch (expects global `tokenizer`)."""
    ...
    # Mask padding labels so cross-entropy ignores them (PyTorch convention: ignore_index=-100).
    pad_mask = (input_ids != tokenizer.pad_token_id).long()
    labels[pad_mask == 0] = -100
    return images, input_ids, labels


# Dataset: loads (image, caption) pairs and returns image tensor + tokenized caption + shifted labels.
class Dataset:
    """Simple image+caption dataset with transforms and GPT-2 tokenization."""

    def __init__(self, df, tfms):
        """Store dataframe and image transforms."""
        self.df = df
        self.tfms = tfms
    ...
    def __getitem__(self, idx):
        """Return (image_tensor, token_ids, shifted_labels) for one captioned image."""
        ...
        # Append EOS so the model learns when to stop generating captions.
...
# Cross-attention: lets text queries attend to image tokens (keys/values) to inject visual context.
class GPT2CrossAttention(nn.Module):
    """Cross-attention: queries from text, keys/values from image tokens."""

    def __init__(self, config):
        """Create cross-attn projections and output projection (GPT-2 style)."""
        super().__init__()
    ...
    def _init_weights(self, module):
        """Initialize Linear weights similarly to GPT-2 defaults."""
    ...
    def forward(self, q, k, v):
        """Compute cross-attention output: Q from text, K/V from image token sequence."""
        ...
        attn_output = attn_weights @ value  # (B, H, T, Dh)
...
# Decoder block: causal self-attn (text) + cross-attn (image) + MLP, with LayerNorm and residuals.
class GPT2Block(nn.Module):
    ...
    def forward(self, hidden_states, encoder_states):
        """Apply causal self-attn (text), cross-attn (image), and MLP with residuals."""
        hidden_states = hidden_states + self.attn(self.ln_1(hidden_states))
        # Cross-attention injects image context into the text stream (Q=text, K/V=image tokens).
...
# VisionGPT2Model: ViT image encoder + GPT-style decoder with cross-attention for caption generation.
class VisionGPT2Model(nn.Module):
    """ViT encoder + GPT-2 style decoder with cross-attention."""

    def __init__(self, config):
        """Initialize ViT image encoder and GPT-style decoder (with cross-attention)."""
        super().__init__()
        
        self.config = config
        
        # ViT encoder (pretrained): produces patch tokens (and CLS token) for the image.
        ...
        # GPT-style text decoder: token+pos embeddings -> decoder blocks -> LM head.
...
# Trainer: training/validation loops, checkpointing, and helper utilities for caption generation demos.
class Trainer:
    """Training loop, validation, and caption generation helpers."""

    def __init__(self,model_config,train_config, dls):
        """Configure model, optimizer/scheduler, AMP, and dataloaders for training."""
...
if __name__ == "__main__":
    ...
    sample_loader = torch.utils.data.DataLoader(train_ds, shuffle=True, batch_size=2, collate_fn=collate_fn)
    ...
    # DataLoaders (use more workers if your disk/CPU can handle it).
    ...
    # Load best checkpoint (update path if you saved elsewhere).
    trainer.load_best_model("/home/rami/OpenUn/LLMs-from-scratch/captioner/captioner.pt")

    # Qualitative demo: show image + compare ground-truth caption vs generated caption.
    for i in range(10):
        det = True
        ...
        t = round(np.random.uniform(0.5, 1.5), 2)
        # if i > 5:
        #     det = True
        gen_caption = trainer.generate_caption(test_img, temperature=t, deterministic=det)
        # Console output (plain print; styling belongs to matplotlib, not print()).
        print(f"actual: {test_caption}\nmodel: {gen_caption}\ntemp: {t} ")

        # Figure title (avoid LaTeX/mathtext like \\color{...}; use matplotlib styling instead).
        plt.title(
            f"actual: {test_caption}\nmodel: {gen_caption}\ntemp: {t} ",
            fontsize=25,
            fontweight="bold",
        )
        # Give the title a bit more room so it doesn't feel clipped.
        plt.tight_layout()
        plt.axis("off")
        plt.show()